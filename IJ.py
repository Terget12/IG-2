...
def fetch_number(chat_id, service_info, msg_id, is_custom=False, retry=0):
    ...
    # শুধুমাত্র সচল প্যানেলগুলো ফিল্টার করা
    active_panels = [p for p in PANELS if p["url"].startswith("http") and p.get("key")]

    # ঠিক যতটুকু নাম্বার দরকার, ততবারই লুপ চলবে
        ...
        for panel in active_panels:
            try:
                ...
                if data.get("success") and data.get("number"):
                    ...
                    number_fetched = True
                    break  # একটি নাম্বার সফলভাবে পেলে ভেতরের লুপ ব্রেক করে পরের নাম্বারের জন্য যাবে
            except Exception as e:
                print(f"❌ [API ERROR] -> {e}")
                continue
        
        # যদি সব প্যানেল চেক করার পরও কোনো নাম্বার না পায় (স্টক আউট), তবে অযথাই আর রিকোয়েস্ট করবে না
        if not number_fetched:
            break

    final_numbers = results

    # যদি একটি নাম্বারও না পাওয়া যায়
    ...
    while time.time() - start < OTP_POLL_TIMEOUT:
        ...
        try:
            ...
            if r.get("success") and r.get("otp"):
                ...
                # ── Private chat-এ OTP message (স্ক্রিনশটের মতো এক লাইনে) ──
                ...
                # Navigation Buttons added below the OTP code
                nav_buttons = []
                if is_custom:
                ...
                # ── Group forward — সিঙ্গেল মেসেজ সিস্টেম (স্ক্রিনশটের মতো) ──
